...
abstract contract LinearCurve is ILinearCurve {
    ...
    /// @inheritdoc ILinearCurve
    function getPrice(uint256 currentSupply, uint256 newSupply) public view returns (uint256 price) {
    ...
    /// @inheritdoc ILinearCurve
    function getCurrentPrice(uint256 currentSupply) public view returns (uint256 price) {
    ...
    function _calculateIntegralAtPoint(uint256 x) internal view returns (UD60x18) {
        return initialPrice.mul(ud(x)).add(slope.mul(pow(ud(x), ud(2e18))).div(ud(2e18)));
    }
}
